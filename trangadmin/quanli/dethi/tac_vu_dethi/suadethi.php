...
<head>
    ...
    <title>Quản Lí</title>
    <link rel="stylesheet" href="suadet.css">
    <link rel="stylesheet"
                ...
                <div class="projects">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div>
                                <button><span class="las la-arrow-left"></span><a href="../quanlidethi.php"
                        ...
                        <?php
                            ...
                            if(isset($_GET["madet"]))
                            {
                                ...
                                $madethi = $_GET["madet"];
                                $_SESSION['madethi'] = $_GET['madet'];
                                // câu lệnh sql
                                $sql = "SELECT * FROM dethi WHERE made = '$madethi'";
                            ...
                            <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="POST">
                                <table>
                                    <tr><td> Mã đề thi </td><td> <input onKeyPress="return isNumberKey(event)" type="text" value = "<?php if(isset($madethi)){ echo $madethi; } ?>" name="manew" required/></td></tr>
                                    <tr><td> Tên đề thi </td><td> <input type="text" value = "<?php if(isset($tende)){ echo $tende; } ?>" name="tennew" required/></td></tr>
                                    <tr><td> Loại đề thi </td><td> <input type="text" value = "<?php if(isset($loaide)){ echo $loaide; } ?>" name="loainew" required/></td></tr>
                            ...
                            <?php
                                if(isset($_POST['nut']))
                                {
                                    $madethicu = $_SESSION['madethi'];
                                    $mademoi = $_POST['manew'];
                                    $tendemoi = $_POST['tennew'];
                                    $loaidemoi = $_POST['loainew'];
                                
                                    $sqlcmd1="SELECT `made` FROM `dethi` WHERE `made` = '$madethicu' ";
                                    $updethi ="UPDATE `dethi` SET `made`='$mademoi',`tende`='$tendemoi',`loaide`='$loaidemoi' WHERE `made` = '$madethicu'";
                                
                                    $kq1 = mysqli_query($conn, $sqlcmd1);
                                    if (mysqli_fetch_array($kq1))
                                    {
                                        $kq2 = mysqli_query($conn, $updethi);
                                        if ($kq2)
                                            echo "<script type='text/javascript'>alert('Sửa thành công đề thi này!');</script>";
                                        else
                                            echo "<script type='text/javascript'>alert('Sửa không thành công đề thi này!');</script>";
                                    }
                                    else
                                        echo "<script type='text/javascript'>alert('Không tồn tại đề thi này!');</script>";
                                }
                            ?>
                        </div>
                    </div>
